-- =============================================================================
-- El Taller — RLS endurecido + user_id + mensajes de contacto + RPCs
-- Ejecutar en: Supabase → SQL Editor → Run
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Helper: ¿es el admin?
-- Ajustá el email si cambia CORREO_ADMIN en admin.html / login.html
-- -----------------------------------------------------------------------------
...
CREATE INDEX IF NOT EXISTS idx_mensajes_contacto_leido
  ON public.mensajes_contacto (leido);
...
-- Cliente: solo los suyos (por user_id; email solo si el pedido guest no tiene user_id)
...
-- Invitados no pueden listar pedidos ajenos (ya no hay SELECT para anon)
-- Admin también cubierto arriba con is_admin()
...
-- =============================================================================
-- STORAGE: solo admin sube/borra (lectura pública se mantiene)
-- =============================================================================
